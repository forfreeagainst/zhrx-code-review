...
> zhrx使用Eslint，制定 JavaScript TypeScript Node 规范
...
子包结构
__tests__ 测试用例
essential: 如果eslint校验太过严格，这里可以降级处理（eg:error变为warn,）
rules: 是各式各样的eslint规则
typescript文件下的vue.js等和index.js、vue.js等都是用来作为入口文件的。
```

## JavaScript 项目 - eslint-config-zhrx
...
### eslint-config-zhrx 安装

```shell
pnpm i -D eslint-config-zhrx @babel/core @babel/eslint-parser eslint-plugin-import
```

### eslint-config-zhrx 配置

```json
{
  "extends": ["eslint-config-zhrx"]
}
```

## JavaScript + React 项目 - eslint-config-zhrx/react
...
### eslint-config-zhrx/react 安装

```shell
npm i -D eslint-config-zhrx @babel/core @babel/eslint-parser eslint-plugin-import eslint-plugin-react eslint-plugin-react-hooks
```

### eslint-config-zhrx/react 配置

```json
{
  "extends": ["eslint-config-zhrx/react"]
}
```
...
```json
{
  "extends": ["eslint-config-zhrx/react", "eslint-config-zhrx/jsx-a11y"]
}
```

## JavaScript + Vue 项目 - eslint-config-zhrx/vue
...
### eslint-config-zhrx/vue 安装

```shell
npm i -D eslint-config-zhrx @babel/core @babel/eslint-parser eslint-plugin-import vue-eslint-parser eslint-plugin-vue
```

### eslint-config-zhrx/vue 配置

```json
{
  "extends": ["eslint-config-zhrx/vue"]
}
```

## JavaScript (Node.js) 项目 - eslint-config-zhrx/node
...
### eslint-config-zhrx/node 安装

```shell
npm i -D eslint-config-zhrx @babel/core @babel/eslint-parser eslint-plugin-import eslint-config-egg
```

### eslint-config-zhrx/node 配置

```json
{
  "extends": ["eslint-config-zhrx/node"]
}
```

## TypeScript 项目 - eslint-config-zhrx/typescript
...
### eslint-config-zhrx/typescript 依赖
...
### eslint-config-zhrx/typescript 安装

```shell
npm i -D eslint-config-zhrx @typescript-eslint/parser @typescript-eslint/eslint-plugin eslint-plugin-import eslint-import-resolver-typescript
```

### eslint-config-zhrx/typescript 配置

```json
{
  "extends": ["eslint-config-zhrx/typescript"]
}
```

注意点：需保证项目已安装 `typescript` 依赖，另外如果项目的 `TS` 配置文件不是 `./tsconfig.json`，则需要设置 `.eslintrc` 中的 [parserOptions.project](https://github.com/typescript-eslint/typescript-eslint/tree/master/packages/parser#parseroptionsproject) 字段 ，例如：
...
## TypeScript + React 项目 - eslint-config-zhrx/typescript/react
...
### eslint-config-zhrx/typescript/react 依赖
...
### eslint-config-zhrx/typescript/react 安装

```shell
npm i -D eslint-config-zhrx @typescript-eslint/parser @typescript-eslint/eslint-plugin eslint-plugin-import eslint-import-resolver-typescript eslint-plugin-react eslint-plugin-react-hooks
```

### eslint-config-zhrx/typescript/react 配置

```json
{
  "extends": ["eslint-config-zhrx/typescript/react"]
}
```
...
```json
{
  "extends": ["eslint-config-zhrx/typescript/react", "eslint-config-zhrx/jsx-a11y"]
}
```

## TypeScript + Vue 项目 - eslint-config-zhrx/typescript/vue
...
### eslint-config-zhrx/typescript/vue 依赖
...
### eslint-config-zhrx/typescript/vue 安装

```shell
npm i -D eslint-config-zhrx @typescript-eslint/parser @typescript-eslint/eslint-plugin eslint-plugin-import eslint-import-resolver-typescript vue-eslint-parser eslint-plugin-vue
```

### eslint-config-zhrx/typescript/vue 配置

```json
{
  "extends": ["eslint-config-zhrx/typescript/vue"]
}
```

## TypeScript (Node.js) 项目 - eslint-config-zhrx/typescript/node
...
### eslint-config-zhrx/typescript/node 依赖
...
### eslint-config-zhrx/typescript/node 安装

```shell
npm i -D eslint-config-zhrx @typescript-eslint/parser @typescript-eslint/eslint-plugin eslint-plugin-import eslint-import-resolver-typescript eslint-config-egg
```

### eslint-config-zhrx/typescript/node 配置

```json
{
  "extends": ["eslint-config-zhrx/typescript/node"]
}
```
...
```json
{
  "extends": ["eslint-config-zhrx/typescript/react", "prettier"]
}
```
...
为了保证一致的编码风格，本包中大量风格相关的规则被设为了 `error` 级别，以引起开发者的足够重视。如果你觉得风格问题不足以是 `error` 级别（有些用户根据 ESLint error 进行流程卡点），本包还提供了一套名为 'essential' 的配置文件，这套配置将所有风格问题降级为 `warn` 级别，仅将必要问题报告为 `error`，引用方式为在相应配置的 `eslint-config-zhrx` 后面加上 `/essential`，如对 `JS React` 项目为 `eslint-config-zhrx/essential/react`、对 `TS Vue` 项目为 `eslint-config-zhrx/essential/typescript/vue`
...
- 了解如何在继承本包的基础上对项目 ESLint 进行个性化配置，可参考官网的 [Configuring ESLint](https://eslint.org/docs/user-guide/configuring)。下面简介下 ESLint 配置中的几个常用字段：
  - `extends`: 继承一组规则集。`"extends": "eslint-config-zhrx",` 表示继承本包定义的规则配置。

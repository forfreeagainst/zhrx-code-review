 ...
 * @param cwd
 * @param pkg
 * @returns eslint-config-zhrx/index
 * @returns eslint-config-zhrx/react
 * @returns eslint-config-zhrx/typescript/index
 * @returns eslint-config-zhrx/typescript/react
 */
export function getESLintConfigType(cwd: string, pkg: PKG): string {
  ...
  return (
    'eslint-config-zhrx/' + `${language}/${dsl}`.replace(/\/$/, '/index').replace(/^\//, '')
  );
}
